          ...
          {dataImage.map((e, i) => {
            return (
              <div key={i} className="relative">
                <Image
                  className="h-[724px] w-[589px] object-obtain rounded-lg cursor-pointer transition-transform duration-300 hover:scale-110"
                  width={286}
                  height={286}
                  src={e.url}
                  alt={e.title}
                ></Image>
                <div className="text-white text-4xl absolute top-10 left-1/2 transform -translate-x-1/2 text-shadow shadow-black">
                  {e.title}
                </div>
              </div>
            );
